...
int main(int argc, char **argv) {
    ...
    char error[512];
    int status = qwen36_m3_run_deltanet_core_benchmark(
        argv[1], warmup, measured, &result, error, sizeof(error));
    if (status != 0) {
        fprintf(stderr, "qwen36 DeltaNet benchmark failed (%d): %s\n",
                status, error);
        return status;
    }
    printf("{\n");
    printf("  \"schema\": 1,\n");
    printf("  \"scope\": \"Qwen3.6-27B one-token DeltaNet recurrent core; projections excluded\",\n");
    printf("  \"device\": \"%s\",\n", result.device_name);
    printf("  \"shape\": {\"key_heads\": %d, \"value_heads\": %d, \"key_dim\": %d, \"value_dim\": %d},\n",
           QWEN36_DELTA_KEY_HEADS, QWEN36_DELTA_VALUE_HEADS,
           QWEN36_DELTA_HEAD_SIZE, QWEN36_DELTA_HEAD_SIZE);
