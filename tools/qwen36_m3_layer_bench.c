...
int main(int argc, char **argv) {
    if (argc < 3 || argc > 5) {
        fprintf(stderr,
                "usage: %s IMAGE.q36m3 METALLIB [MEASURED] [WARMUP]\n",
                argv[0]);
        return 2;
    }
    ...
    char error[512];
    int status = qwen36_m3_run_layer_benchmark(
        argv[2], argv[1], warmup, measured, &result, error, sizeof(error));
    if (status != 0) {
        fprintf(stderr, "qwen36 layer benchmark failed (%d): %s\n",
                status, error);
        return status;
    }
    printf("{\n");
    printf("  \"schema\": 1,\n");
    printf("  \"scope\": \"Qwen3.6-27B real layer-0 one-token decode; not full-model tokens/s\",\n");
    printf("  \"device\": \"%s\",\n", result.device_name);
