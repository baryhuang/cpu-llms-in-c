...
static int parse_vocab(const char *json, token_string *tokens) {
    ...
    for (;;) {
        ...
        if (parse_u32(&vocab, &id) != 0 ||
            id >= QWEN36_TOKENIZER_BASE_VOCAB ||
            tokens[id].data != NULL) {
            free(text.data);
            return -1;
        }
    ...
    return count == QWEN36_TOKENIZER_BASE_VOCAB ? 0 : -1;
}
...
static int parse_added(const char *json, token_string *tokens) {
    ...
    ++cursor;
    size_t count = 0;
    while (count < QWEN36_TOKENIZER_ADDED_COUNT) {
        ...
        if (parse_u32(&id_key, &id) != 0 ||
            ...
            id < QWEN36_TOKENIZER_ADDED_FIRST ||
            id >= QWEN36_TOKENIZER_VOCAB || tokens[id].data != NULL) {
            free(content.data);
            return -1;
        }
        tokens[id] = content;
...
static int compare_merges(const void *left, const void *right) {
    const qwen36_token_merge_entry *a = left;
    const qwen36_token_merge_entry *b = right;
...
static int parse_merges(const char *json, const uint32_t *hash_table,
                        token_string *tokens,
                        qwen36_token_merge_entry *merges) {
    const char *model = strstr(json, "\"model\"");
    const char *cursor =
        model != NULL ? strstr(model, "\"merges\"") : NULL;
    if (cursor == NULL || (cursor = strchr(cursor, '[')) == NULL) return -1;
    ++cursor;
    for (uint32_t rank = 0; rank < QWEN36_TOKENIZER_MERGES; ++rank) {
        skip_space(&cursor);
        if (*cursor == ',') {
            ++cursor;
            skip_space(&cursor);
        ...
        merges[rank] = (qwen36_token_merge_entry){
            .left = left_id, .right = right_id,
            .result = result_id, .rank = rank
        };
        free(left.data); free(right.data); free(combined);
    }
    qsort(merges, QWEN36_TOKENIZER_MERGES, sizeof(*merges),
          compare_merges);
    for (size_t index = 1; index < QWEN36_TOKENIZER_MERGES; ++index) {
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr, "usage: %s tokenizer.json OUTPUT.q36tok\n", argv[0]);
        return 2;
    }
    ...
    close(input_file);
    json[json_length] = '\0';
    qwen36_sha256_context sha;
    unsigned char digest[32];
    char actual[65];
    qwen36_sha256_init(&sha);
    qwen36_sha256_update(&sha, json, json_length);
    qwen36_sha256_final(&sha, digest);
    ...
    token_string *tokens =
        calloc(QWEN36_TOKENIZER_VOCAB, sizeof(*tokens));
    uint32_t *hash_table = calloc(VOCAB_HASH_SIZE, sizeof(*hash_table));
    qwen36_token_merge_entry *merges =
        malloc((size_t)QWEN36_TOKENIZER_MERGES * sizeof(*merges));
    ...
    for (uint32_t id = 0; id < QWEN36_TOKENIZER_VOCAB; ++id) {
    ...
    qwen36_tokenizer_image_header header;
    memset(&header, 0, sizeof(header));
    memcpy(header.magic, QWEN36_TOKENIZER_MAGIC, 8);
    ...
    header.token_entry_bytes = sizeof(qwen36_token_directory_entry);
    ...
    qwen36_token_directory_entry *directory =
        calloc(QWEN36_TOKENIZER_VOCAB, sizeof(*directory));
    token_string *decoded =
        calloc(QWEN36_TOKENIZER_VOCAB, sizeof(*decoded));
    if (directory == NULL || decoded == NULL) return 5;
    uint64_t blob_bytes = 0;
    for (uint32_t id = 0; id < QWEN36_TOKENIZER_VOCAB; ++id) {
        int result = id < QWEN36_TOKENIZER_BASE_VOCAB ?
        ...
        directory[id].flags =
            id >= QWEN36_TOKENIZER_ADDED_FIRST ? 1u : 0u;
        blob_bytes += decoded[id].length;
    }
    header.token_directory_offset = QWEN36_TOKENIZER_HEADER_BYTES;
    header.token_directory_bytes =
        (uint64_t)QWEN36_TOKENIZER_VOCAB * sizeof(*directory);
    ...
    header.merges_bytes =
        (uint64_t)QWEN36_TOKENIZER_MERGES * sizeof(*merges);
    ...
    for (uint32_t id = 0; id < QWEN36_TOKENIZER_VOCAB && !failed; ++id) {
    ...
    printf("{\"source\":\"%s\",\"source_sha256\":\"%s\","
           ...
           QWEN36_TOKENIZER_VOCAB, QWEN36_TOKENIZER_MERGES);
    return 0;
}
