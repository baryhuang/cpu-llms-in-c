...
static int add_attention_layer(
    ...
    const qwen36_m3_attention_image_header *header) {
    if (memcmp(header->magic, QWEN36_M3_ATTENTION_IMAGE_MAGIC, 8) != 0 ||
        header->version != QWEN36_M3_ATTENTION_IMAGE_VERSION ||
        header->layer_index != layer || header->hidden_size != 5120 ||
...
static int add_global(const qwen36_m3_global_image_header *header) {
    if (memcmp(header->magic, QWEN36_M3_GLOBAL_IMAGE_MAGIC, 8) != 0 ||
        header->version != QWEN36_M3_GLOBAL_IMAGE_VERSION ||
...
int main(int argc, char **argv) {
    ...
    char path[1024];
    snprintf(path, sizeof(path), "%s/global.q36global", argv[1]);
    if (map_source(0, path) != 0 ||
        add_global((const qwen36_m3_global_image_header *)
                       sources[0].mapping) != 0) {
    ...
    for (unsigned layer = 0; layer < 64; ++layer) {
        const char *suffix = layer % 4 == 3 ? "q36att" : "q36delta";
        ...
        int status = layer % 4 == 3 ?
            add_attention_layer(
                layer, source,
                (const qwen36_m3_attention_image_header *)
                    sources[source].mapping) :
            add_delta_layer(
                layer, source,
                (const qwen36_m3_image_header *)sources[source].mapping);
        if (status != 0) {
