...
int main(int argc, char **argv) {
    ...
    if (argc < first_number || argc > first_number + 2) {
        fprintf(stderr, "usage: %s [--image IMAGE.q36m3] METALLIB "
    ...
    char error[512];
    int status = qwen36_m3_run_mlp_microbenchmark(metallib_path, image_path,
                                                  warmup, measured,
                                                  &result, error, sizeof(error));
    if (status != 0) {
        fprintf(stderr, "qwen36 M3 Pro benchmark failed (%d): %s\n", status, error);
        return status;
    }

    printf("{\n");
    printf("  \"schema\": 2,\n");
    printf("  \"scope\": \"Qwen3.6-27B layer-0 MLP primitive; not full-model tokens/s\",\n");
    printf("  \"device\": \"%s\",\n", result.device_name);
    printf("  \"weight_source\": \"%s\",\n", result.weight_source);
    printf("  \"shape\": {\"hidden\": %d, \"intermediate\": %d, \"group_size\": %d},\n",
           QWEN36_HIDDEN_SIZE, QWEN36_MLP_SIZE, QWEN36_Q4_GROUP_SIZE);
