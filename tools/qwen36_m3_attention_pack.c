...
static int validate_matrix(const qwen36_tensor_view *view, const char *name,
...
int main(int argc, char **argv) {
    if (argc != 5 || strlen(argv[3]) != 64) {
        fprintf(stderr, "usage: %s SOURCE.safetensors OUTPUT.q36att "
...
#undef MAKE_NAME
    ...
    qwen36_tensor_view gate_w, gate_s, gate_b, up_w, up_s, up_b;
    qwen36_tensor_view down_w, down_s, down_b;
    qwen36_tensor_view input_norm, post_norm, q_norm, k_norm;
    qwen36_tensor_view q_w, q_s, q_b, k_w, k_s, k_b, v_w, v_s, v_b;
    qwen36_tensor_view o_w, o_s, o_b;
...
#define VM(view, name, dtype, rows, cols, bytes) \
    ...
    if (VM(gate_w, GATE_W, "U32", QWEN36_MLP_SIZE, 640,
           mlp_weight_bytes) ||
        VM(up_w, UP_W, "U32", QWEN36_MLP_SIZE, 640, mlp_weight_bytes) ||
        VM(down_w, DOWN_W, "U32", QWEN36_HIDDEN_SIZE, 2176,
           mlp_weight_bytes) ||
        VM(gate_s, GATE_S, "BF16", QWEN36_MLP_SIZE, hidden_groups,
           mlp_meta_plane) ||
        VM(gate_b, GATE_B, "BF16", QWEN36_MLP_SIZE, hidden_groups,
           mlp_meta_plane) ||
        VM(up_s, UP_S, "BF16", QWEN36_MLP_SIZE, hidden_groups,
           mlp_meta_plane) ||
        VM(up_b, UP_B, "BF16", QWEN36_MLP_SIZE, hidden_groups,
           mlp_meta_plane) ||
        VM(down_s, DOWN_S, "BF16", QWEN36_HIDDEN_SIZE, mlp_groups,
           mlp_meta_plane) ||
        VM(down_b, DOWN_B, "BF16", QWEN36_HIDDEN_SIZE, mlp_groups,
           mlp_meta_plane) ||
        VM(q_w, Q_W, "U32", QWEN36_ATTENTION_Q_ROWS, 640,
           q_weight_bytes) ||
        VM(q_s, Q_S, "BF16", QWEN36_ATTENTION_Q_ROWS, hidden_groups,
           (uint64_t)QWEN36_ATTENTION_Q_ROWS * hidden_groups * 2) ||
        VM(q_b, Q_B, "BF16", QWEN36_ATTENTION_Q_ROWS, hidden_groups,
           (uint64_t)QWEN36_ATTENTION_Q_ROWS * hidden_groups * 2) ||
        VM(k_w, K_W, "U32", QWEN36_ATTENTION_K_ROWS, 640,
           kv_weight_bytes) ||
        VM(k_s, K_S, "BF16", QWEN36_ATTENTION_K_ROWS, hidden_groups,
           (uint64_t)QWEN36_ATTENTION_K_ROWS * hidden_groups * 2) ||
        VM(k_b, K_B, "BF16", QWEN36_ATTENTION_K_ROWS, hidden_groups,
           (uint64_t)QWEN36_ATTENTION_K_ROWS * hidden_groups * 2) ||
        VM(v_w, V_W, "U32", QWEN36_ATTENTION_V_ROWS, 640,
           kv_weight_bytes) ||
        VM(v_s, V_S, "BF16", QWEN36_ATTENTION_V_ROWS, hidden_groups,
           (uint64_t)QWEN36_ATTENTION_V_ROWS * hidden_groups * 2) ||
        VM(v_b, V_B, "BF16", QWEN36_ATTENTION_V_ROWS, hidden_groups,
           (uint64_t)QWEN36_ATTENTION_V_ROWS * hidden_groups * 2) ||
        VM(o_w, O_W, "U32", QWEN36_HIDDEN_SIZE, 768, o_weight_bytes) ||
        VM(o_s, O_S, "BF16", QWEN36_HIDDEN_SIZE, 96,
           (uint64_t)QWEN36_HIDDEN_SIZE * 96 * 2) ||
        VM(o_b, O_B, "BF16", QWEN36_HIDDEN_SIZE, 96,
           (uint64_t)QWEN36_HIDDEN_SIZE * 96 * 2) ||
        validate_vector(&input_norm, INPUT_NORM, QWEN36_HIDDEN_SIZE) ||
        validate_vector(&post_norm, POST_NORM, QWEN36_HIDDEN_SIZE) ||
        validate_vector(&q_norm, Q_NORM, QWEN36_ATTENTION_HEAD_SIZE) ||
        validate_vector(&k_norm, K_NORM, QWEN36_ATTENTION_HEAD_SIZE)) {
        return 4;
    }
#undef VM
