...
Usage:
  tools/qwen38_monitor.py                  # watch the next qwen38* process
  tools/qwen38_monitor.py python3.13       # watch mlx-lm / oMLX runs
  tools/qwen38_monitor.py 12345            # watch a specific pid
  tools/qwen38_monitor.py -i 0.5 --record run.jsonl
  tools/qwen38_monitor.py --render run.jsonl -o run.html
...
def run_dashboard(sampler, arguments, record_file):
    ...
    try:
        while True:
            ...
            frame.append(
                f"qwen38 monitor  target '{sampler.target}'  "
...
<title>qwen38 monitor — {target}</title>
<style>
:root {{
  color-scheme: light;
...
def main():
    ...
    parser.add_argument("target", nargs="?", default="qwen38",
