...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr, "usage: %s TOKENIZER.q36tok UTF8_TEXT\n", argv[0]);
        return 2;
    }
    char error[256];
    qwen36_tokenizer *tokenizer =
        qwen36_tokenizer_open(argv[1], error, sizeof(error));
    if (tokenizer == NULL) {
    ...
    size_t count = 0;
    if (ids == NULL || qwen36_tokenizer_encode(
    ...
    if (decoded == NULL || qwen36_tokenizer_decode(
