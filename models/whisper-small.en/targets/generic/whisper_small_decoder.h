#ifndef CLLM_WHISPER_SMALL_DECODER_H
#define CLLM_WHISPER_SMALL_DECODER_H

#include <stddef.h>
#include <stdint.h>

#define CLLM_WHISPER_SMALL_DECODER_LAYERS 12U
#define CLLM_WHISPER_SMALL_DECODER_CONTEXT 448U
#define CLLM_WHISPER_SMALL_VOCABULARY 51864U
...
typedef struct {
    ...
    cllm_whisper_matrix self_query;
    const float *self_query_bias;
    cllm_whisper_matrix self_key;
    cllm_whisper_matrix self_value;
    const float *self_value_bias;
    cllm_whisper_matrix self_output;
    const float *self_output_bias;

    const float *cross_norm_weight;
    const float *cross_norm_bias;
    cllm_whisper_matrix cross_query;
    const float *cross_query_bias;
    cllm_whisper_matrix cross_key;
    cllm_whisper_matrix cross_value;
    const float *cross_value_bias;
    cllm_whisper_matrix cross_output;
    const float *cross_output_bias;

    const float *mlp_norm_weight;
    const float *mlp_norm_bias;
    cllm_whisper_matrix mlp_input;
    const float *mlp_input_bias;
    cllm_whisper_matrix mlp_output;
    const float *mlp_output_bias;
} cllm_whisper_decoder_layer_weights;

typedef struct {
    cllm_whisper_matrix token_embedding;
 ...
 * `hidden_out`, when non-null, receives the final normalized [768] state.
 */
