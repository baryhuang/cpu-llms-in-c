...
int main(int argc,char **argv)
{
    ...
    minimindo_audio_encoder_stream *stream=
        minimindo_audio_encoder_stream_open(model,error,sizeof(error));
    if(!stream||minimindo_parallel_session_begin(4U)!=0)return 4;
    int encode_result=0;
    for(size_t offset=0;offset<sample_count&&encode_result==0;offset+=512){
        size_t count=sample_count-offset;if(count>512)count=512;
        size_t produced=0;
        encode_result=minimindo_audio_encoder_stream_push_pcm16(
            stream,samples+offset,count,0,output+frames*768,
            (capacity-frames)*768,&produced,error,sizeof(error));
        frames+=produced;
    }
    if(!encode_result){
        size_t produced=0;
        encode_result=minimindo_audio_encoder_stream_push_pcm16(
            stream,NULL,0,1,output+frames*768,(capacity-frames)*768,
            &produced,error,sizeof(error));
        frames+=produced;
    }
    minimindo_parallel_session_end();
    minimindo_audio_encoder_stream_close(stream);
    ...
    printf("{\"samples\":%zu,\"frames\":%zu,\"rms\":%.9g,\"peak\":%.9g,\"streaming\":true}\n",sample_count,frames,sqrt(sum/(frames*768)),peak);
