...
static int validate_attention_image(
    const qwen36_m3_attention_image_header *h, size_t length) {
    const uint64_t mlp_weight =
        (uint64_t)QWEN36_MLP_SIZE * QWEN36_HIDDEN_SIZE / 2;
    const uint64_t mlp_meta =
        (uint64_t)QWEN36_MLP_SIZE * 80 * 4;
    const uint64_t attention_input_q =
        (uint64_t)QWEN36_ATTENTION_INPUT_ROWS * QWEN36_HIDDEN_SIZE / 2;
    const uint64_t attention_input_m =
        (uint64_t)QWEN36_ATTENTION_INPUT_ROWS * 80 * 4;
    const uint64_t attention_output_q =
        (uint64_t)QWEN36_HIDDEN_SIZE * 6144 / 2;
    ...
    if (memcmp(h->magic, QWEN36_M3_ATTENTION_IMAGE_MAGIC, 8) != 0 ||
        ...
        h->layer_index % 4 != 3 || h->hidden_size != QWEN36_HIDDEN_SIZE ||
...
static void encode_attention_layer(
    ...
    const qwen36_attention_pipelines *p,
    const qwen36_attention_buffers *b,
    const qwen36_m3_attention_image_header *h) {
    qwen36_attention_parameters parameters = {
    ...
    [encoder dispatchThreadgroups:
        MTLSizeMake((QWEN36_ATTENTION_INPUT_ROWS + 7) / 8, 1, 1)
            threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
...
static BOOL submit_attention(
    id<MTLCommandQueue> queue, const qwen36_attention_pipelines *p,
    const qwen36_attention_buffers *b,
    const qwen36_m3_attention_image_header *h, unsigned iterations) {
...
static float attention_q4_half(const uint8_t *q,
                               const qwen36_attention_q4_meta *m,
...
static int attention_cpu_reference(
    ...
    const qwen36_attention_q4_meta *input_m =
        (const qwen36_attention_q4_meta *)(
    ...
    const qwen36_attention_q4_meta *output_m =
        (const qwen36_attention_q4_meta *)(
    ...
    const qwen36_attention_q4_meta *gate_m =
        (const qwen36_attention_q4_meta *)(image + h->gate_metadata_offset);
    const uint8_t *up_q = image + h->up_quants_offset;
    const qwen36_attention_q4_meta *up_m =
        (const qwen36_attention_q4_meta *)(image + h->up_metadata_offset);
    ...
    const qwen36_attention_q4_meta *down_m =
        (const qwen36_attention_q4_meta *)(image + h->down_metadata_offset);
...
int qwen36_m3_run_attention_layer_benchmark(
    ...
    @autoreleasepool {
        ...
        unsigned char *image = mmap(NULL, image_length, PROT_READ,
                                    MAP_PRIVATE, file, 0);
        const qwen36_m3_attention_image_header *h =
            (const qwen36_m3_attention_image_header *)image;
        if (image == MAP_FAILED ||
...
#define PIPE(field, name) \
        ...
        qwen36_attention_pipelines p = {
            PIPE(rms_half, @"qwen36_rmsnorm_f16_to_f16"),
            PIPE(rms_float, @"qwen36_rmsnorm_f32_to_f16"),
            PIPE(input_projection, @"qwen36_q4_attention_inputs"),
            PIPE(prepare_query, @"qwen36_attention_prepare_query"),
            PIPE(prepare_key_value, @"qwen36_attention_prepare_key_value"),
            PIPE(scores, @"qwen36_attention_scores"),
            PIPE(softmax_value, @"qwen36_attention_softmax_value_gate"),
            PIPE(output_projection,
                 @"qwen36_q4_attention_output_residual"),
            PIPE(mlp_gate_up, @"qwen36_q4_gate_up_silu"),
            PIPE(mlp_down, @"qwen36_q4_mlp_down_residual_f32")
        };
#undef PIPE
