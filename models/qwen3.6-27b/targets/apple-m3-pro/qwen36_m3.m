...
typedef struct __attribute__((packed, aligned(2))) {
    ...
    uint16_t scale;
    uint16_t bias;
} qwen36_q4_interleaved_group;

_Static_assert(sizeof(qwen36_q4_meta) == 4,
               "CPU and Metal Q4 metadata strides must match");
_Static_assert(sizeof(qwen36_q4_interleaved_group) == QWEN36_Q4_GROUP_BYTES,
...
static void fill_weights(uint8_t *quants, qwen36_q4_meta *metadata,
                         size_t group_count,
                         uint32_t seed) {
    uint32_t state = seed;
    for (size_t index = 0; index < group_count; ++index) {
        for (size_t byte = 0; byte < QWEN36_Q4_GROUP_SIZE / 2; ++byte) {
            quants[index * (QWEN36_Q4_GROUP_SIZE / 2) + byte] =
...
static void interleave_weights(qwen36_q4_interleaved_group *output,
                               const uint8_t *quants,
                               const qwen36_q4_meta *metadata,
                               size_t group_count) {
    for (size_t index = 0; index < group_count; ++index) {
        memcpy(output[index].q,
               quants + index * (QWEN36_Q4_GROUP_SIZE / 2),
               QWEN36_Q4_GROUP_SIZE / 2);
...
static void encode_matvec(id<MTLCommandBuffer> command,
    ...
    MTLSize groups = MTLSizeMake((QWEN36_MLP_SIZE + simdgroups - 1) / simdgroups, 1, 1);
...
static void encode_silu(id<MTLCommandBuffer> command,
    ...
    [encoder setBuffer:output offset:0 atIndex:2];
    [encoder dispatchThreads:MTLSizeMake(QWEN36_MLP_SIZE, 1, 1)
...
static void encode_down(id<MTLCommandBuffer> command,
    ...
    NSUInteger simdgroups = threads / 32;
    MTLSize groups = MTLSizeMake((QWEN36_HIDDEN_SIZE + simdgroups - 1) /
                                 simdgroups, 1, 1);
...
int qwen36_m3_run_mlp_microbenchmark(const char *metallib_path,
    ...
    @autoreleasepool {
        ...
        id<MTLComputePipelineState> matvec = make_pipeline(library, @"qwen36_q4_matvec", &error);
        id<MTLComputePipelineState> interleaved =
            make_pipeline(library, @"qwen36_q4_interleaved_matvec", &error);
        id<MTLComputePipelineState> fused = make_pipeline(library, @"qwen36_q4_gate_up_silu", &error);
        id<MTLComputePipelineState> silu = make_pipeline(library, @"qwen36_silu_mul", &error);
        id<MTLComputePipelineState> down =
            make_pipeline(library, @"qwen36_q4_down_residual", &error);
        ...
        const size_t groups_per_row = QWEN36_HIDDEN_SIZE / QWEN36_Q4_GROUP_SIZE;
        const size_t group_count = (size_t)QWEN36_MLP_SIZE * groups_per_row;
        const size_t quant_bytes = group_count * (QWEN36_Q4_GROUP_SIZE / 2);
        const size_t metadata_bytes = group_count * sizeof(qwen36_q4_meta);
        const size_t matrix_bytes = quant_bytes + metadata_bytes;
        const size_t down_group_count = (size_t)QWEN36_HIDDEN_SIZE *
                                        (QWEN36_MLP_SIZE /
                                         QWEN36_Q4_GROUP_SIZE);
        const size_t down_quant_bytes = down_group_count *
                                        (QWEN36_Q4_GROUP_SIZE / 2);
        const size_t down_metadata_bytes = down_group_count *
                                           sizeof(qwen36_q4_meta);
        const size_t down_matrix_bytes = down_quant_bytes + down_metadata_bytes;
        const size_t vector_bytes = (size_t)QWEN36_HIDDEN_SIZE * sizeof(uint16_t);
        const size_t output_bytes = (size_t)QWEN36_MLP_SIZE * sizeof(float);
        ...
        id<MTLBuffer> full_mlp_output =
            [device newBufferWithLength:QWEN36_HIDDEN_SIZE * sizeof(float)
                                options:MTLResourceStorageModeShared];
        int image_file = -1;
        void *image_mapping = MAP_FAILED;
        size_t image_length = 0;
        const qwen36_m3_image_header *image_header = NULL;
        if (image_path != NULL) {
            ...
            if (memcmp(image_header->magic, QWEN36_M3_IMAGE_MAGIC, 8) != 0 ||
                ...
                image_header->down_rows != QWEN36_HIDDEN_SIZE ||
                ...
                set_error(error_message, error_message_capacity,
                          @"invalid Qwen3.6 M3 image header or length");
        ...
        const qwen36_q4_meta *gate_meta_data = gate_metadata.contents;
        const uint8_t *up_quant_data = up_quants.contents;
        const qwen36_q4_meta *up_meta_data = up_metadata.contents;
