...
static BOOL run_direct(id<MTLCommandQueue> queue,
    ...
    NSUInteger count = QWEN36_DELTA_VALUE_HEADS * QWEN36_DELTA_HEAD_SIZE;
...
static BOOL run_vectorized(id<MTLCommandQueue> queue,
    ...
    [encoder setBuffer:state offset:0 atIndex:5];
    [encoder setBuffer:output offset:0 atIndex:6];
    NSUInteger count = QWEN36_DELTA_VALUE_HEADS *
                       (QWEN36_DELTA_HEAD_SIZE / 2);
    [encoder dispatchThreads:MTLSizeMake(count, 1, 1)
          threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
    [encoder endEncoding];
    [command commit];
...
static void fill_inputs(float *query, float *key, float *value,
    ...
    for (unsigned head = 0; head < QWEN36_DELTA_VALUE_HEADS; ++head) {
        double query_squared = 0.0;
        double key_squared = 0.0;
        for (unsigned index = 0; index < QWEN36_DELTA_HEAD_SIZE; ++index) {
            size_t position = (size_t)head * QWEN36_DELTA_HEAD_SIZE + index;
        ...
        float query_scale = 1.0f /
            sqrtf((float)(query_squared + 1.0e-6) * QWEN36_DELTA_HEAD_SIZE);
    ...
    const size_t state_count =
        (size_t)QWEN36_DELTA_VALUE_HEADS * QWEN36_DELTA_HEAD_SIZE *
...
static void cpu_reference(const float *query, const float *key,
    ...
    for (unsigned head = 0; head < QWEN36_DELTA_VALUE_HEADS; ++head) {
        size_t vector_base = (size_t)head * QWEN36_DELTA_HEAD_SIZE;
        size_t state_head_base = vector_base * QWEN36_DELTA_HEAD_SIZE;
        for (unsigned value_index = 0; value_index < QWEN36_DELTA_HEAD_SIZE;
             ++value_index) {
            float kv_memory = 0.0f;
            float previous_output = 0.0f;
            float key_query = 0.0f;
            for (unsigned key_index = 0; key_index < QWEN36_DELTA_HEAD_SIZE;
                 ++key_index) {
                size_t state_index = state_head_base +
                    (size_t)key_index * QWEN36_DELTA_HEAD_SIZE + value_index;
...
int qwen36_m3_run_deltanet_core_benchmark(
    ...
    @autoreleasepool {
        ...
        id<MTLComputePipelineState> direct = library != nil ?
            make_pipeline(library, @"qwen36_deltanet_direct", &error) : nil;
        id<MTLComputePipelineState> vectorized = library != nil ?
            make_pipeline(library, @"qwen36_deltanet_float2_vectorized", &error) : nil;
        ...
        const size_t state_count = vector_count * QWEN36_DELTA_HEAD_SIZE;
        const size_t state_bytes = state_count * sizeof(float);
        const size_t scalar_bytes = QWEN36_DELTA_VALUE_HEADS * sizeof(float);
