...
typedef struct __attribute__((packed, aligned(2))) {
    uint16_t scale;
    uint16_t bias;
} qwen36_layer_q4_meta;

_Static_assert(sizeof(qwen36_layer_q4_meta) == 4,
               "CPU and Metal metadata strides must match");

enum {
    QWEN36_LAYER_CONSTANTS = 51424,
    QWEN36_LAYER_THREADS = 256,
    QWEN36_LAYER_SIMDGROUPS = QWEN36_LAYER_THREADS / 32
};
...
static int validate_layer_image(const qwen36_m3_image_header *header,
                                size_t length) {
    const uint64_t mlp_weight_bytes =
        (uint64_t)QWEN36_MLP_SIZE * QWEN36_HIDDEN_SIZE / 2;
    const uint64_t mlp_metadata_bytes =
        (uint64_t)QWEN36_MLP_SIZE *
        (QWEN36_HIDDEN_SIZE / QWEN36_Q4_GROUP_SIZE) * 4;
    const uint64_t delta_input_quants =
        (uint64_t)QWEN36_DELTA_INPUT_ROWS * QWEN36_HIDDEN_SIZE / 2;
    const uint64_t delta_input_metadata_payload =
        (uint64_t)QWEN36_DELTA_INPUT_ROWS *
        (QWEN36_HIDDEN_SIZE / QWEN36_Q4_GROUP_SIZE) * 4;
    const uint64_t delta_output_quants =
        (uint64_t)QWEN36_HIDDEN_SIZE *
        QWEN36_DELTA_OUTPUT_INPUTS / 2;
    const uint64_t delta_output_metadata =
        (uint64_t)QWEN36_HIDDEN_SIZE *
        (QWEN36_DELTA_OUTPUT_INPUTS / QWEN36_Q4_GROUP_SIZE) * 4;
    uint64_t expected = QWEN36_M3_IMAGE_HEADER_BYTES;
#define Q36_CHECK_SEGMENT(name, bytes) do { \
    if (header->name##_offset != expected || \
        header->name##_bytes != (bytes)) return -1; \
    expected += (bytes); \
} while (0)
    if (memcmp(header->magic, QWEN36_M3_IMAGE_MAGIC, 8) != 0 ||
        ...
        header->delta_input_rows != QWEN36_DELTA_INPUT_ROWS ||
        ...
        header->delta_output_groups_per_row !=
            QWEN36_DELTA_OUTPUT_INPUTS / QWEN36_Q4_GROUP_SIZE ||
        header->constants_f32_count != QWEN36_LAYER_CONSTANTS ||
        memcmp(header->source_sha256, QWEN36_M3_EXPECTED_SOURCE_SHA256,
               QWEN36_M3_SOURCE_SHA256_LENGTH) != 0) {
        return -1;
    }
    Q36_CHECK_SEGMENT(gate_quants, mlp_weight_bytes);
    Q36_CHECK_SEGMENT(gate_metadata, mlp_metadata_bytes);
    Q36_CHECK_SEGMENT(up_quants, mlp_weight_bytes);
    Q36_CHECK_SEGMENT(up_metadata, mlp_metadata_bytes);
    Q36_CHECK_SEGMENT(down_quants, mlp_weight_bytes);
    Q36_CHECK_SEGMENT(down_metadata, mlp_metadata_bytes);
    expected = layer_align_page(expected);
    if (header->constants_offset != expected ||
        header->constants_bytes !=
            layer_align_page(QWEN36_LAYER_CONSTANTS * sizeof(float)) ||
    ...
    Q36_CHECK_SEGMENT(delta_input_quants, delta_input_quants);
    Q36_CHECK_SEGMENT(delta_input_metadata,
                      layer_align_page(delta_input_metadata_payload));
    Q36_CHECK_SEGMENT(delta_output_quants, delta_output_quants);
    Q36_CHECK_SEGMENT(delta_output_metadata, delta_output_metadata);
#undef Q36_CHECK_SEGMENT
...
static void encode_q4_rows(id<MTLComputeCommandEncoder> encoder,
    ...
    NSUInteger groups = (rows + QWEN36_LAYER_SIMDGROUPS - 1) /
                        QWEN36_LAYER_SIMDGROUPS;
    [encoder dispatchThreadgroups:MTLSizeMake(groups, 1, 1)
            threadsPerThreadgroup:MTLSizeMake(QWEN36_LAYER_THREADS, 1, 1)];
}

static void encode_one_layer(id<MTLComputeCommandEncoder> encoder,
                             const qwen36_layer_pipelines *p,
                             const qwen36_layer_buffers *b,
                             const qwen36_m3_image_header *h) {
    ...
    encode_q4_rows(encoder, p->delta_inputs, b->normalized,
                   ...
                   b->projected, QWEN36_DELTA_INPUT_ROWS);
    ...
    [encoder dispatchThreads:MTLSizeMake(QWEN36_DELTA_QKV_ROWS, 1, 1)
            threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
    ...
    [encoder dispatchThreadgroups:MTLSizeMake(QWEN36_DELTA_VALUE_HEADS, 1, 1)
    ...
    [encoder dispatchThreads:
        MTLSizeMake(QWEN36_DELTA_VALUE_HEADS * QWEN36_DELTA_HEAD_SIZE, 1, 1)
            threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
    ...
    [encoder dispatchThreadgroups:
        MTLSizeMake((QWEN36_HIDDEN_SIZE + QWEN36_LAYER_SIMDGROUPS - 1) /
                    QWEN36_LAYER_SIMDGROUPS, 1, 1)
            threadsPerThreadgroup:MTLSizeMake(QWEN36_LAYER_THREADS, 1, 1)];
    ...
    [encoder dispatchThreadgroups:
        MTLSizeMake((QWEN36_MLP_SIZE + QWEN36_LAYER_SIMDGROUPS - 1) /
                    QWEN36_LAYER_SIMDGROUPS, 1, 1)
            threadsPerThreadgroup:MTLSizeMake(QWEN36_LAYER_THREADS, 1, 1)];
...
static BOOL submit_layers(id<MTLCommandQueue> queue,
                          const qwen36_layer_pipelines *pipelines,
                          const qwen36_layer_buffers *buffers,
                          const qwen36_m3_image_header *header,
                          unsigned iterations) {
...
static float cpu_q4_dot_half(const uint8_t *quants,
                             const qwen36_layer_q4_meta *metadata,
...
static void cpu_rms_half(const uint16_t *input, const float *weight,
    ...
    float inv = 1.0f /
        sqrtf((float)(squared / QWEN36_HIDDEN_SIZE) + 1.0e-6f);
    for (size_t index = 0; index < QWEN36_HIDDEN_SIZE; ++index) {
...
static int cpu_layer_reference(const unsigned char *image,
    ...
    uint16_t *normalized = malloc(QWEN36_HIDDEN_SIZE * sizeof(uint16_t));
    float *projected = malloc(QWEN36_DELTA_INPUT_ROWS * sizeof(float));
    float *convolved = malloc(QWEN36_DELTA_QKV_ROWS * sizeof(float));
    const size_t delta_vector_count =
        QWEN36_DELTA_VALUE_HEADS * QWEN36_DELTA_HEAD_SIZE;
    ...
    float *mixer_output = malloc(QWEN36_HIDDEN_SIZE * sizeof(float));
    uint16_t *post = malloc(QWEN36_HIDDEN_SIZE * sizeof(uint16_t));
    float *intermediate = malloc(QWEN36_MLP_SIZE * sizeof(float));
    ...
    const qwen36_layer_q4_meta *delta_input_meta =
        (const qwen36_layer_q4_meta *)(
            image + h->delta_input_metadata_offset);
    for (size_t row = 0; row < QWEN36_DELTA_INPUT_ROWS; ++row) {
    ...
    memset(conv_state_out, 0,
           QWEN36_DELTA_QKV_ROWS * QWEN36_DELTA_CONV_SIZE * sizeof(float));
    for (size_t channel = 0; channel < QWEN36_DELTA_QKV_ROWS; ++channel) {
    ...
    float decay[QWEN36_DELTA_VALUE_HEADS];
    float beta[QWEN36_DELTA_VALUE_HEADS];
    const float *a_log = constants + h->a_log_constants_index;
    const float *dt_bias = constants + h->dt_bias_constants_index;
    for (size_t head = 0; head < QWEN36_DELTA_VALUE_HEADS; ++head) {
    ...
    const qwen36_layer_q4_meta *delta_output_meta =
        (const qwen36_layer_q4_meta *)(
            image + h->delta_output_metadata_offset);
    for (size_t row = 0; row < QWEN36_HIDDEN_SIZE; ++row) {
    ...
    const qwen36_layer_q4_meta *gate_meta =
        (const qwen36_layer_q4_meta *)(image + h->gate_metadata_offset);
    const uint8_t *up_quants = image + h->up_quants_offset;
    const qwen36_layer_q4_meta *up_meta =
        (const qwen36_layer_q4_meta *)(image + h->up_metadata_offset);
    for (size_t row = 0; row < QWEN36_MLP_SIZE; ++row) {
    ...
    const qwen36_layer_q4_meta *down_meta =
        (const qwen36_layer_q4_meta *)(image + h->down_metadata_offset);
...
int qwen36_m3_run_layer_benchmark(
    ...
    @autoreleasepool {
        ...
        if (image_file < 0 || fstat(image_file, &image_status) != 0 ||
            image_status.st_size < QWEN36_M3_IMAGE_HEADER_BYTES) {
        ...
        const qwen36_m3_image_header *header =
            (const qwen36_m3_image_header *)image;
        ...
        const size_t half_hidden_bytes =
            QWEN36_HIDDEN_SIZE * sizeof(uint16_t);
        const size_t float_hidden_bytes =
            QWEN36_HIDDEN_SIZE * sizeof(float);
        const size_t projected_bytes =
            QWEN36_DELTA_INPUT_ROWS * sizeof(float);
        const size_t conv_state_bytes =
            QWEN36_DELTA_QKV_ROWS * QWEN36_DELTA_CONV_SIZE * sizeof(float);
        const size_t convolved_bytes =
            QWEN36_DELTA_QKV_ROWS * sizeof(float);
        const size_t vector_bytes =
            QWEN36_DELTA_VALUE_HEADS * QWEN36_DELTA_HEAD_SIZE *
            sizeof(float);
        const size_t state_bytes =
            QWEN36_DELTA_VALUE_HEADS * QWEN36_DELTA_HEAD_SIZE *
            QWEN36_DELTA_HEAD_SIZE * sizeof(float);
        const size_t scalar_bytes =
            QWEN36_DELTA_VALUE_HEADS * sizeof(float);
        const size_t intermediate_bytes =
            QWEN36_MLP_SIZE * sizeof(float);
...
#define Q36_MAPPED_BUFFER(field) \
        ...
        qwen36_layer_buffers b = {
            ...
            .delta_input_quants = Q36_MAPPED_BUFFER(delta_input_quants),
            .delta_input_metadata = Q36_MAPPED_BUFFER(delta_input_metadata),
            ...
            .delta_output_quants = Q36_MAPPED_BUFFER(delta_output_quants),
            .delta_output_metadata = Q36_MAPPED_BUFFER(delta_output_metadata),
            .mixer_output = [device newBufferWithLength:float_hidden_bytes
                                                options:shared],
            .post_normalized = [device newBufferWithLength:half_hidden_bytes
                                                   options:shared],
            .gate_quants = Q36_MAPPED_BUFFER(gate_quants),
            .gate_metadata = Q36_MAPPED_BUFFER(gate_metadata),
            .up_quants = Q36_MAPPED_BUFFER(up_quants),
            .up_metadata = Q36_MAPPED_BUFFER(up_metadata),
            .down_quants = Q36_MAPPED_BUFFER(down_quants),
            .down_metadata = Q36_MAPPED_BUFFER(down_metadata),
