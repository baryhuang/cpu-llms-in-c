...
int main(int argc, char **argv) {
    ...
    for (size_t run = 0; run < sizeof(runs) / sizeof(runs[0]); ++run) {
        ...
        /* Candidate: batched prefill for tokens 0..count-1, then the same
         * logits probe. */
        ...
        if (qwen36_m3_model_prefill(model, sequence, count, 0, &prefill,
        ...
        int pass = nan_count == 0 && existence_mismatch == 0 &&
                   ...
                   (prefill.chunk32_count != 0 || bitwise);
        printf("check=run%u chunks=32x%u/16x%u/1x%u bitwise_states=%s "
               "bitwise_logits=%s state_max_abs=%.9g nan=%u "
               "argmax=%u/%u status=%s\n",
               count, prefill.chunk32_count, prefill.chunk16_count,
               prefill.single_count, bitwise ? "yes" : "no",
               logits_bitwise ? "yes" : "no", max_abs, nan_count,
               reference_best, candidate_best, pass ? "PASS" : "FAIL");
        if (!pass) ++failures;
