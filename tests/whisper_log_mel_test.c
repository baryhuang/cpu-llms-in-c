...
int main(int argc, char **argv)
{
    ...
    if (data == NULL || size < HEADER_BYTES || memcmp(data, "WHMEL001", 8U) != 0 ||
        read_u32(data + 8U) != 1U) {
        fprintf(stderr, "error: invalid Whisper log-Mel fixture\n");
        free(data);
        return 2;
    }
    ...
    if (read_u32(data + 20U) != CLLM_WHISPER_N_FFT ||
        ...
        read_u32(data + 28U) != CLLM_WHISPER_LARGE_V3_N_MELS ||
        read_u32(data + 32U) != CLLM_WHISPER_N_FREQUENCIES ||
        frames != cllm_whisper_log_mel_frames(samples)) {
        fprintf(stderr, "error: incompatible Whisper log-Mel fixture dimensions\n");
        free(data);
        return 2;
    }

    audio_bytes = (size_t)samples * sizeof(float);
    filter_count = CLLM_WHISPER_LARGE_V3_N_MELS * CLLM_WHISPER_N_FREQUENCIES;
    filter_bytes = filter_count * sizeof(float);
    output_count = (size_t)frames * CLLM_WHISPER_LARGE_V3_N_MELS;
    output_bytes = output_count * sizeof(float);
    expected_size = HEADER_BYTES + audio_bytes + filter_bytes + output_bytes;
    if (size != expected_size) {
        fprintf(stderr, "error: Whisper log-Mel fixture size mismatch\n");
        free(data);
        return 2;
    }
    ...
    workspace = malloc(cllm_whisper_log_mel_workspace_floats(samples) * sizeof(float));
    if (actual == NULL || workspace == NULL ||
        cllm_whisper_log_mel_128(audio, samples, filters, actual, workspace,
                                 cllm_whisper_log_mel_workspace_floats(samples)) != 0) {
        fprintf(stderr, "error: Whisper log-Mel execution failed\n");
        free(actual);
        free(workspace);
        free(data);
        return 2;
    ...
    printf("boundary=log_mel_128 count=%zu max_abs=%.9g max_rel=%.9g status=%s\n",
