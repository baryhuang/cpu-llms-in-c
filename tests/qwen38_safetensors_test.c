...
int main(void) {
    ...
    char path[] = "/tmp/qwen38-safetensors-XXXXXX";
    ...
    char error[256];
    int status = qwen38_safetensors_find(path, "tensor", 1, &view,
                                         error, sizeof(error));
    ...
    status = qwen38_safetensors_find(path, "tenso", 1, &view,
                                     error, sizeof(error));
    if (status == 0) {
    ...
    close(file);
    status = qwen38_safetensors_find(path, "tensor", 1, &view,
                                     error, sizeof(error));
    if (status == 0) {
    ...
    status = qwen38_safetensors_find(path, "tensor", 0, &view,
                                     error, sizeof(error));
    unlink(path);
    if (status != 0) {
    ...
    puts("qwen38 safetensors reader: ok");
    return 0;
}
